...
EOSIO_TEST_BEGIN(symbol_code_type_test)
   ...
   CHECK_EQUAL( !symbol_code{0ULL}, true )
   CHECK_EQUAL( !symbol_code{1ULL}, false )

   CHECK_EQUAL( symbol_code{""}.operator bool(), false )
   CHECK_EQUAL( symbol_code{"SYMBOL"}.operator bool(), true )
   CHECK_EQUAL( !symbol_code{""}, true )
   CHECK_EQUAL( !symbol_code{"SYMBOL"}, false )
...
EOSIO_TEST_BEGIN(symbol_type_test)
   ...
   CHECK_EQUAL( !symbol{0}, true )
   CHECK_EQUAL( !symbol{1}, false )

   CHECK_EQUAL( (symbol{"", 0}.operator bool()), false )
   CHECK_EQUAL( (symbol{"SYMBOLL", 0}.operator bool()), true )
   CHECK_EQUAL( (!symbol{"", 0}), true )
   CHECK_EQUAL( (!symbol{"SYMBOLL", 0}), false )
   ...
   // CHECK_PRINT( ",A", [&](){symbol{"A", 0}.print(false);} );
...
EOSIO_TEST_BEGIN(extended_symbol_type_test)
   ...
   CHECK_EQUAL( (extended_symbol{s2, n4}.get_symbol().raw()), 4702111234474983680ULL )
   CHECK_EQUAL( (extended_symbol{s2, n5}.get_symbol().code().raw()), 18367622009667905ULL )
   CHECK_EQUAL( (extended_symbol{s3, n6}.get_symbol().raw()), 6510615555426900480ULL )
   ...
   // Must fix the behavior of `print` in order for these tests to work
   // ---------------------
   // void print(bool)const
   // extended_symbol{s0, n0}.print(true);
   // extended_symbol{s0, n0}.print(true);
   // CHECK_PRINT( "0@576460752303423488", [](){extended_symbol{s0, n0}.print(true);} );
   // CHECK_PRINT( "0@2882303761517117440", [](){extended_symbol{s1, n1}.print(true);} );
   // CHECK_PRINT( "255@595056260442243615", [](){extended_symbol{symbol{"AAAAAAA", 255}, name{"111111111111j"}}.print(true);} );
   // CHECK_PRINT( "255@2975281302211218015", [](){extended_symbol{symbol{"ZZZZZZZ", 255}, name{"555555555555j"}}.print(true);} );

   // CHECK_PRINT( "@576460752303423488", [](){extended_symbol{symbol{"A", 0}, name{"1"}}.print(false);} );
   ...
   // CHECK_PRINT( "@2975281302211218015", [](){extended_symbol{symbol{"ZZZZZZZ", 255}, name{"555555555555j"}}.print(false);} );
