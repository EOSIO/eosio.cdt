...
#define EOSIO_CDT_AUTO_MEM(NAME, SZ, MAX_SZ) \
   auto NAME = SZ > MAX_SZ ? detail::scoped_memory(SZ) : detail::scoped_memory(alloca(SZ));
...
namespace detail {
   ...
   struct scoped_memory {
      explicit scoped_memory(std::size_t sz) : mem(malloc(sz)), should_free(true) {}
      explicit scoped_memory(void* m) : mem(m), should_free(false) {}
      ~scoped_memory() { if (should_free) free(mem); }
      void* value() { return mem; }
      void* mem = nullptr;
      bool should_free = true;
   };
}

inline static constexpr name kv_ram_storage = "eosio.kvram"_n;
inline static constexpr name kv_disk_storage = "eosio.kvdisk"_n;
...
inline key_type make_key(const char* str, size_t size) {
   ...
   EOSIO_CDT_AUTO_MEM(mem, data_size, detail::max_stack_buffer_size);
   void* data_buffer = mem.value();
...
template <uint8_t PrefixBits = 0x1>
inline key_type make_prefix(eosio::name table_name, eosio::name index_name=""_n) {
   key_type ret;
   uint8_t status = PrefixBits;
   if constexpr (PrefixBits == 0x1) {
      ...
      ret = make_key(std::make_tuple(0x2, table_name.value));
   }
   return ret;
}
/* @endcond */
 ...
 * @tparam T         - the type of the data stored as the value of the table
  */
template<typename T, uint8_t PrefixBits=0x1 /*do not set unless you know what you are doing*/>
class kv_table {
   ...
   public:
      ...
      T value() const {
         ...
         EOSIO_CDT_AUTO_MEM(mem, value_size, detail::max_stack_buffer_size);
         void* buffer = mem.value();
         ...
         if (!is_primary) {
            ...
            EOSIO_CDT_AUTO_MEM(mem, actual_value_size, detail::max_stack_buffer_size);
            buffer = mem.value();
            internal_use_do_not_use::kv_get_data(index->tbl->db_name, 0, (char*)buffer, actual_data_size);

            return unpack<T>((char*)buffer, actual_data_size);
         }

         return unpack<T>((char*)buffer, actual_value_size);
      }
   ...
   public:
      ...
      std::optional<T> get(const K& key) const {
         ...
         if (!is_primary) {
            ...
            EOSIO_CDT_AUTO_MEM(mem, actual_data_size, detail::max_stack_buffer_size);
            void* pk_buffer = mem.value();
            auto pk_copy_size = internal_use_do_not_use::kv_get_data(tbl->db_name, 0, (char*)pk_buffer, actual_data_size);

            return unpack<T>((char*)pk_buffer, pk_copy_size);
         }

         return unpack<T>((char*)buffer, copy_size);
      }
...
protected:
   kv_table() = default;
   explicit kv_table(eosio::name tn) : table_name(tn) {}
   kv_table(eosio::name cn, eosio::name tn) : contract_name(cn), table_name(tn) {}
   kv_table(eosio::name cn, eosio::name tn, eosio::name dbn) : contract_name(cn), table_name(tn), db_name(dbn.value) {}
   ...
   eosio::name get_contract_name()const { return contract_name; }
...
template <typename T, eosio::name::raw SingletonName, eosio::name::raw DefaultContractName=""_n, eosio::name::raw StorageType = kv_ram_storage>
class kv_singleton : private kv_table<T, 0x2> {
   using kv_table_t = kv_table<T, 0x2>;
   public:
      kv_singleton()
         : kv_table_t(DefaultContractName, name{static_cast<uint64_t>(SingletonName)}, name{static_cast<uint64_t>(StorageType)}), key(make_prefix(SingletonName)){}

      kv_singleton(eosio::name contract_name)
         : kv_table_t(contract_name, name{static_cast<uint64_t>(SingletonName)}, name{static_cast<uint64_t>(StorageType)}), key(make_prefix(SingletonName)) {}

      ~kv_singleton() {
         write_back<false>();
      }

      const T& get()const { return get_cached(); }

      T& get() { return get_cached(); }

      void set(const T& v) {
         modify([&](auto& val) { val = v; });
      }

      template <typename Lambda>
      void modify(Lambda&& modifier) {
         modifier(value());
         is_cached = true;
      }

      void erase() {
         kv_table_t::erase(value());
         is_cached = false;
      }

      template <bool FullCleanup=true>
      void write_back() {
         kv_table_t::put(value());
      }

   private:
      key_type make_prefix(eosio::name::raw nm)const { return make_key(std::make_tuple(0x2, static_cast<uint64_t>(SingletonName))); }
      key_type key;
      ...
      T& get_cached() {
         if (!is_cached) {
            uint32_t sz = 0;
            internal_use_do_not_use::kv_get(static_cast<uint64_t>(StorageType), kv_table_t::get_contract_name().value, key.data(), key.size(), sz);
            EOSIO_CDT_AUTO_MEM(mem, sz, detail::max_stack_buffer_size);
            char* data_buffer = (char*)mem.value();
            internal_use_do_not_use::kv_get_data(static_cast<uint64_t>(StorageType), 0, data_buffer, sz);
            value() = unpack<T>(data_buffer, sz);
            is_cached = true;
         }
         return value();
      }
...
#undef EOSIO_CDT_GET_RETURN_T
#undef EOSIO_CDT_AUTO_MEM
