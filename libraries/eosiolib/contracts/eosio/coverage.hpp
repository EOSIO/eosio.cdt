...
namespace eosio {
   ...
   inline void coverage_inc_fun_cnt( name code, uint32_t file_num, uint32_t func_num ) {
       internal_use_do_not_use::coverage_inc_fun_cnt(static_cast<uint64_t>(code), file_num, func_num);
   }

   inline void coverage_inc_line_cnt( name code, uint32_t file_num, uint32_t line_num ) {
       internal_use_do_not_use::coverage_inc_line_cnt(static_cast<uint64_t>(code), file_num, line_num);
   }

   inline uint32_t coverage_get_fun_cnt( name code, uint32_t file_num, uint32_t func_num ) {
       return internal_use_do_not_use::coverage_get_fun_cnt(static_cast<uint64_t>(code), file_num, func_num);
   }

   inline uint32_t coverage_get_line_cnt( name code, uint32_t file_num, uint32_t line_num ) {
       return internal_use_do_not_use::coverage_get_line_cnt(static_cast<uint64_t>(code), file_num, line_num);
   }    
