...
namespace cosmwasm {
   std::unique_ptr<region> build_region_from_components(ptr_t offset, uint32_t capacity, uint32_t length) {
      return std::make_unique<region>(offset, capacity, length);
   }

   region* alloc(size_t size) {
      void* data = malloc(size);
      return build_region_from_components((ptr_t)data, (uint32_t)size, 0).release();
   }

   std::unique_ptr<region> build_region(char* data, uint32_t length) {
      return build_region_from_components((ptr_t)data, length, length);
   }
   ...
   std::unique_ptr<region> build_region_dup(char* data, uint32_t length) {
      ...
      return build_region_from_components((ptr_t)dup, length, length);
   }
