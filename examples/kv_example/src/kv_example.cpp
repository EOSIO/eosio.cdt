#include <kv_example.hpp>
[[eosio::action]] 
void kv_example::set( name user ) {
...
[[eosio::action]] 
void kv_example::print( name user ) {
   auto itr = testtab.find(user.value);
...
[[eosio::action]] 
void kv_example::bysec( name secid ) {
...
[[eosio::action]] 
void kv_example::mod( name user, uint32_t n ) {
   auto itr = testtab.find(user.value);
   check( itr != testtab.end(), "user does not exist in table" );
