...
eval $DOCKER_PULL
echo "Done with pull"
echo "docker run $PROXY_DOCKER_RUN_ARGS --rm -v "$(pwd):/eos" -w '/eos' -it $DOCKER_REPO/$IMAGE ./.cicd/test-package.run.sh"
...
docker run $PROXY_DOCKER_RUN_ARGS --rm -v "$(pwd):/eos" -w '/eos' -it $DOCKER_REPO/$IMAGE ./.cicd/test-package.run.sh
